...
void draw (t_p *table, int size_l, int size_c, t_data *data)
{
	...
	while(l < size_l)
	{
		...
		while (c < size_c)
		{
			start = table[l * size_l + c];
			if(c != (size_c - 1))
			{
				p_droite = table[l * size_l + c + 1];
				bresenham(start, p_droite, data, COLOR_RED);
			}
			if(l < (size_l - 1))
			{
				p_bas = table[(l + 1) * size_l + c];
...
/* ************************************************************************** */
	...
	// l = 0; c = 0;	start = table[l * size_l + c];
	// l = 0; c = 1;	end = table[l * size_l + c];
	// bresenham(start, end, data, COLOR_RED);
	
	// l = 0; c = 0;	start = table[l * size_l + c];
	// l = 1; c = 0;	end = table[l * size_l + c];
	// bresenham(start, end, data, COLOR_RED);
	
	// l = 1; c = 0;	start = table[l * size_l + c];
	// l = 1; c = 1;	end = table[l * size_l + c];
	// bresenham(start, end, data, COLOR_RED);
	
	// l = 0; c = 1;	start = table[l * size_l + c];
	// l = 1; c = 1;	end = table[l * size_l + c];
	// bresenham(start, end, data, COLOR_RED);
